...
<?php foreach ($comments as $k => $v): ?>
    <div id = "pad">
        <div class = "meta">
            <h4>
                <?php entities($v->username) ?><div style = "text-align: right"><?php entities($v->created) ?></div>
            </h4>
        </div>
        <div class = "well" style = "width: 900px"><?php echo entities($v->body) ?>
        </div>
    </div>
<?php endforeach ?>
...
<hr>
    <div id = "pad">
        <form class = "meta" method = "post" action = "<?php entities(url('comment/write')) ?>">
            <div style = "color:#0080FF"><?php echo entities($_SESSION['username']) ?></div><br />
